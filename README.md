...
Light mode is **fully automatic**, including the cross-volume case. If your main checkout sits on NTFS but the machine has a Dev Drive (ReFS) on another letter, worktwin detects it on the first `/worktwin` spawn from each source branch and parks an implicit base **as a linked git worktree** at `<dev-drive>:\.worktwin-bases\<repo-name>__<from-branch-slug>` (detached HEAD, shares the main's git object store, no second `.git` database). Every worker spawned from the same source branch is a CoW reflink from that base; spawning in parallel from `main` and `release/1.4` writes to two different bases, so there is no race on `git reset --hard FROM_REF` between source branches. When `/worktwin-clear` removes the last worker tied to a given base, that base is removed too, so the Dev Drive ends up clean when no work is in flight. Zero config: you keep your main checkout where you want it, and the parallel work happens on the fast volume. If you ever need to override, tell the agent to "force light" or "force heavy" and the skill will pass `--force-light` or `--force-heavy` to `worktwin-init`. The output JSON reports which path was used. Full details in [docs/light-mode.md](docs/light-mode.md).
