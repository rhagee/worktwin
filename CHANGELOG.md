...
- **Auto-discovered per-source-branch base on every CoW-capable target, with reference-counted cleanup.** `worktwin-init` now detects when the main repo lives on a non-CoW filesystem (Windows NTFS being the common case) and a Dev Drive is available on the system. On the first `/worktwin` spawn from a given source branch worktwin auto-creates an implicit base **as a linked git worktree** at `<parent-of-base>/.worktwin-bases/<repo-name>__<from-branch-slug>` (detached HEAD, shares the main's object store, no second git database). The base location depends on the main repo's filesystem:

- **Main on CoW (Case 1: macOS APFS, Linux btrfs/XFS, Windows main on ReFS)**: the base sits next to the main and its working tree is created via CoW reflink from the main, so the base costs ~0 bytes when main HEAD already matches FROM_REF (and only the delta otherwise). This guarantees Case 1 is at least as cheap as Case 2.
- **Main on non-CoW (Case 2: Windows NTFS)**: the base sits on the Dev Drive and pays one full working-tree checkout per active from_branch.

In both cases workers off the same from_branch CoW-reflink from the base; workers from different from_branches get their own base, so spawning in parallel from `main` and `release/1.4` writes to two different directories and races on `git reset --hard FROM_REF` are impossible by construction. **No manual setup**: the user keeps their main checkout wherever they want, runs `/worktwin`, and every worker lands on a CoW-capable volume with zero-overhead reflinks regardless of source branch. When `/worktwin-clear` removes the last worker tied to a given base, that base is removed too (per-base reference counting), so the auto-managed `.worktwin-bases` directory ends up empty when no work is in flight. The previously-available `worktwin-light-base set` mapping is no longer consulted by `worktwin-init`; it stays installed for back-compat but the auto path is the canonical one.
