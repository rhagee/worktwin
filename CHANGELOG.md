...
- **Auto-discovery of the Dev Drive at spawn time, with reference-counted cleanup.** `worktwin-init` now detects when the main repo lives on a non-CoW filesystem (Windows NTFS being the common case) and a Dev Drive is available on the system. On the first `/worktwin` spawn for that repo it auto-creates an implicit base **as a linked git worktree** of the main repo at `<dev-drive>:\.worktwin-bases\<repo-name>` (detached HEAD, shares the main's object store, no second git database on disk). From the second spawn onward the base is reused silently and worker worktrees CoW-reflink from it. **No manual setup**: the user keeps their main checkout wherever they want, runs `/worktwin`, and worktwin parks the worker on the fast volume with zero-overhead reflinks. When `/worktwin-clear` removes the last worker that referenced the base, the base worktree itself is removed too, so the Dev Drive is automatically clean when no work is in flight. The previously-required `worktwin-light-base set` step is now optional - it stays available for power users who want a custom mapping, but the typical path is fully automatic.
