...
- **Auto-discovery of the Dev Drive at spawn time, per source branch, with reference-counted cleanup.** `worktwin-init` now detects when the main repo lives on a non-CoW filesystem (Windows NTFS being the common case) and a Dev Drive is available on the system. On the first `/worktwin` spawn from a given source branch it auto-creates an implicit base **as a linked git worktree** at `<dev-drive>:\.worktwin-bases\<repo-name>__<from-branch-slug>` (detached HEAD, shares the main's object store, no second git database). Workers spawned from the same source branch CoW-reflink from that base; workers from a different source branch get their own base, so spawning in parallel from `main` and `release/1.4` writes to two different directories and races on `git reset --hard FROM_REF` are impossible by construction. **No manual setup**: the user keeps their main checkout wherever they want, runs `/worktwin`, and every worker lands on the fast volume with zero-overhead reflinks regardless of source branch. When `/worktwin-clear` removes the last worker tied to a given base, that base is removed too (per-base reference counting), so the Dev Drive ends up clean when no work is in flight. The previously-available `worktwin-light-base set` mapping is no longer consulted by `worktwin-init`; it stays installed for back-compat but the auto path is the canonical one.
