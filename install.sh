...
mkdir -p "$TARGET"
for skill in worktwin worktwin-ship worktwin-ship-all worktwin-finalize worktwin-status worktwin-clear worktwin-light-doctor worktwin-help worktwin-update; do
  rm -rf "$TARGET/$skill"
...
if [ -d "$BIN_SRC" ]; then
  ...
  mkdir -p "$BIN_DST"
  cp "$BIN_SRC"/* "$BIN_DST/"
  chmod +x "$BIN_DST"/worktwin-init "$BIN_DST"/worktwin-claude-md "$BIN_DST"/worktwin-list "$BIN_DST"/worktwin-clear "$BIN_DST"/worktwin-light-check "$BIN_DST"/worktwin-help "$BIN_DST"/worktwin-update 2>/dev/null || true
fi
